    ...
    <div className="flex justify-between flex-col w-full">
      <div className="w-full bg-black">
        <div className={`container flex justify-between w-full mx-auto xl:py-2 py-[9px] lg:items-center items-start`}>
          ...
          <select className=" cursor-pointer custom-select outline-none text-xs px-1 bg-black text-white flex justify-center items-center appearance-none" name="Eng" id="">
      ...
      <div className="container flex justify-between w-full mx-auto lg:pt-[13px] pt-4 items-center relative">
        ...
        <div className={`flex xl:gap-[43px] xl:pl-10 lg:pl-5 items-center max-lg:px-4 relative w-full max-lg:bg-black mx-auto lg:gap-7 gap-6 !text-black lg:max-h-max max-lg:fixed max-lg:top-0 max-lg:h-full max-lg:w-full max-lg:flex-col max-lg:bg-hero-pattern max-lg:duration-300 justify-center max-lg:items-center z-[60] ${open ? "max-lg:left-0" : "max-lg:left-full"}`}>
          {HEADER_LIST.map((item, index) => (
            <Link  onClick={()=>setOpen(!open)} key={index} href={item.link} className="relative lg:text-sm text-xl font-medium text-black transition-all duration-300 ease-linear group"> {item.title}
